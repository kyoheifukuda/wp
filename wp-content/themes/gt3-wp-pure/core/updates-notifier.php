...
{
    global $gt3_un_theme_pref;
    $update_notify_system['checkers']['last_check_id_relation_file'] = time();
    ...
    if (isset($array->$gt3_un_theme_pref)) {
        $update_notify_system['theme_tf_id'] = $array->$gt3_un_theme_pref;
    }
    gt3_un_update_option('update_notify_system', $update_notify_system);
    return true;
}
...
{
    global $update_notify_system;
    $wp_remote_tf_info = wp_remote_get('http://marketplace.envato.com/api/v3/item:' . $update_notify_system['theme_tf_id'] . '.json');
    $update_notify_system['checkers']['last_check_tf_update_field'] = time();

    $gt3_un_my_theme = wp_get_theme();
    $new_theme_version = $gt3_un_my_theme->get('Version');
    $update_notify_system['checkers']['theme_version_on_check_moment'] = $new_theme_version;
    ...
    if (is_array($wp_remote_tf_info)) {
        ...
        if (!isset($update_notify_system['checkers']['theme_last_update_on_check_moment'])) {
            $update_notify_system['checkers']['theme_last_update_on_check_moment'] = $new_last_update;
        }
        if ($update_notify_system['checkers']['theme_last_update_on_check_moment'] !== $new_last_update) {
            gt3_un_start_showing_notify();
        }
    }

    if ($update_notify_system['checkers']['theme_version_on_check_moment'] !== $new_theme_version) {
...
if (GT3_THEME_UPDATES_NOTIFIER_ENABLE == true) {
    $update_notify_system = gt3_un_get_option('update_notify_system');

    # Check for the relation
    if (isset($update_notify_system['theme_tf_id'])) {
        if (!isset($update_notify_system['checkers']['last_check_tf_update_field']) || (time() - $update_notify_system['checkers']['last_check_tf_update_field'] > 86400)) {
            gt3_un_check_info_from_tf();
        }
    } else {
        if (!isset($update_notify_system['checkers']['last_check_id_relation_file']) || (time() - $update_notify_system['checkers']['last_check_id_relation_file'] > 86400)) {
