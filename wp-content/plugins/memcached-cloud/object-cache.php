<?php
/*
Plugin Name: Memcached Cloud
Description: Memcached backend for WP Object Cache, using binary protocol and SASL authentication. Memcached 2.2.0 PECL package (http://pecl.php.net/package/memcached) is required.
Version: 1.0
Plugin URI: 
...
class WP_Object_Cache {
	...
	public function __construct( $persistent_id = NULL ) {
		...
		if ( isset( $memcached_servers ) )
			...
			$this->servers =  array( explode(':', getenv("MEMCACHEDCLOUD_SERVERS") ) );		
		
		if ( isset( $memcached_username ) )
			$this->username = $memcached_username;
		else
			$this->username = getenv('MEMCACHEDCLOUD_USERNAME');
			
		if ( isset( $memcached_password ) )
			$this->password = $memcached_password;
		else
			$this->password = getenv('MEMCACHEDCLOUD_PASSWORD');
